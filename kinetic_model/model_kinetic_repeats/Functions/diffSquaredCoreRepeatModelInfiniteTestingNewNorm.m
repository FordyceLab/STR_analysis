function [squaredDiffTot] = diffSquaredCoreRepeatModelInfiniteTestingNewNorm(params,koffData,KDData,weights,seqIdentifier,paramIndex,KDNorm,koffNorm)
...
for i=1:numInData
    for j=1:numReps
        if weights(i)>0
            ...
            squaredDiffsKD(i,j)=(((KDModel(i)-KDData(i,j))/KDNorm)^2)*weights(i);
            squaredDiffskoff(i,j)=(((koffModel(i)-koffData(i,j))/koffNorm)^2)*weights(i);
            else
