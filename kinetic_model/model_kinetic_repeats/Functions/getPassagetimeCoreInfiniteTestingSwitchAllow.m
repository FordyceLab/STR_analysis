function [onCoreFirstPassage] = getPassagetimeCoreInfiniteTestingSwitchAllow(params,solutionConc)
%returns first passage time from dissocatiated to the core for the 4 state
%model defined by
%params = [konmax koffmicro_core koffmicro_flank p_core_rel p_flank_rel k_switch]
...
%paramsOrig=params;
%Adding a a switching rate k_swithc, directly from flank to core, without
%going through testing state

p_core = params(4)/(1+params(4)+params(5));
p_flank= params(5)/(1+params(4)+params(5));
p_switch=params(6)/(params(3)+params(6));

t1E = 1/(params(1)*solutionConc);
t4E = 1/(params(3)+params(6));
onCoreFirstPassage = ((1-p_flank*p_switch)*(t1E)+p_flank*(t4E))/(p_core+p_flank-p_flank*p_switch);
